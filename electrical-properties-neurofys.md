...
$$V_m(l)=V_{max}(1-e^{-l/\lambda})$$
where: $$\lambda=\sqrt{\frac{r_m}{r_i+r_o}}\approx\sqrt{\frac{r_m}{r_i}}$$
We see that when $x=\lambda$, then $V_m(\lambda)=V_{max}(1-e^{-1})$, meaning that we see an $\approx 37\%$ decline in membrane potential at distance of the length constant.
